// ========================================================
// MOBILE RESPONSIVE CSS INJECTION (NEW in v1.6.3)
// ========================================================
const style = document.createElement('style');
style.innerHTML = `
    /* Reset dasar biar rapi di HP */
    ...
    /* Styling Canvas agar fit di layar HP tanpa kepotong */
    ...
    /* CSS Media Query Khusus untuk Handphone */
    @media (max-width: 600px) {
        #game-container { width: 95vw; height: 95vw; } /* Canvas jadi 95% lebar layar */
        #battleCanvas { border-width: 2px; }
        
        /* Penyesuaian UI Overlay Victory/Pause biar pas di layar kecil */
        #overlay-msg { font-size: 28px !important; text-align: center; width: 90%; }
        
        /* Skill Tooltip di HP taruh di bawah aja biar gak nutupin jari */
        #skill-tooltip { 
        ...
        #skill-tooltip b { font-size: 11px !important; }
        #skill-tooltip span { font-size: 9px !important; }
    }
`;
...
const BUILD_VER = "v1.6.3";

// Ukuran dasar arena 500x500 (CSS yang bakal scaling ini ke mobile)
...
// Skill Data (English)
const skillDetails = {
    'Human': { passive: 'Spam Mastery', pDesc: 'Mana cap: <b style="color:#ffdd59">30 Mana</b>, frequent Ultimate casts.', ulti: 'Physical Burst', uDesc: 'Next collision: <b style="color:#ff5e57">+3 DMG</b>. 1x Hit.' },
    'Naruto': { passive: 'Infinite Army', pDesc: 'No clone limit. Each active clone grants <b style="color:#ffdd59">+8% Speed</b>.', ulti: 'Kage Bunshin', uDesc: 'Summons <b style="color:#ffdd59">2 Clones</b> (HP:10, DMG:3).' },
    'Gojo': { passive: 'Infinity Aura', pDesc: 'Enemies within <b style="color:#0fbcf9">100px</b> slowed by <b style="color:#ffdd59">90%</b>.', ulti: 'Unlimited Void', uDesc: 'Heal: +8 HP. Freeze all: <b style="color:#ffdd59">4s</b>. Takes <b style="color:#ff5e57">1 DMG</b>.' },
    'Sukuna': { passive: 'Giant Fire Arrow', pDesc: 'Auto-fires deals <b style="color:#ff5e57">7 DMG</b> every <b style="color:#ffdd59">5s</b>.', ulti: 'Malevolent Shrine', uDesc: ' bloody domain deals <b style="color:#ff5e57">2 DMG</b> per tick. Screen shake.' },
    'Pain': { passive: 'Bansho Tenin', pDesc: 'Every <b style="color:#ffdd59">4 hits</b>, pulls enemies within <b style="color:#0fbcf9">90px</b>.', ulti: 'Almighty Push', uDesc: 'Gravity blast <b style="color:#0fbcf9">450px</b> area. <b style="color:#ffdd59">12.0 Push Power</b>.' }
};
...
// Variabel Scaling untuk Mobile (Default PC)
let scaleFactor = 1.0; 
let baseUnitRadius = 35;
let baseProjectileRadius = 16;
let baseHPFontSize = 22; // BESAR (v1.6.3)
...
class Projectile {
    constructor(x, y, targetX, targetY, dmg, ownerIdx) {
        ...
        this.radius = baseProjectileRadius * scaleFactor; // Responsive Radius
        this.speed = 7;
...
class Unit {
    constructor(name, hp, dmg, speed, color, startX, startY, playerIdx, isClone = false) {
        ...
        this.radius = baseUnitRadius * scaleFactor; // Responsive Radius
    ...
    update(deltaTime) {
        ...
        if (distCenter < GLOBAL_GRAVITY_RADIUS && distCenter > 10) { this.x += (dxCenter / distCenter) * GLOBAL_GRAVITY_POWER; this.y += (dyCenter / distCenter) * GLOBAL_GRAVITY_POWER; }
        ...
        if (this.x - this.radius < arenaLeft) { this.x = arenaLeft + this.radius; this.dirX *= -1; } if (this.x + this.radius > arenaRight) { this.x = arenaRight - this.radius; this.dirX *= -1; } if (this.y - this.radius < arenaTop) { this.y = arenaTop + this.radius; this.dirY *= -1; } if (this.y + this.radius > arenaBottom) { this.y = arenaBottom - this.radius; this.dirY *= -1; }
    }
    ...
    draw(ctx) {
        ...
        ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); ctx.fillStyle = (this.hitTimer > 0) ? "white" : c; ctx.fill(); ctx.strokeStyle = "white"; ctx.lineWidth = 2 * scaleFactor; ctx.stroke(); ctx.restore();
        if (this.isStunned) { ctx.fillStyle = "yellow"; ctx.font = `${20 * scaleFactor}px Arial`; ctx.fillText("💫", this.x, this.y - (45 * scaleFactor)); }
        
        // --- BUFFED HP TEXT VISUALS (v1.6.3) ---
        let hpVal = Math.round(Math.max(0, this.hp));
        let hpFontSize = baseHPFontSize * scaleFactor;
        ctx.font = `bold ${hpFontSize}px sans-serif`; // Bold & Besar
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        
        // Stroke Hitam
        ctx.strokeStyle = "black";
        ctx.lineWidth = 4 * scaleFactor;
        ctx.strokeText(hpVal, this.x, this.y);
        
        // Fill Putih
        ctx.fillStyle = "white";
        ctx.fillText(hpVal, this.x, this.y);
    }
}

// ========================================================
// RESPONSIVE SCALING LOGIC (NEW in v1.6.3)
// ========================================================
function adjustScaling() {
    // Cek lebar layar (viewport)
    const screenWidth = window.innerWidth;
    
    if (screenWidth < 600) {
        // Mode HP: Scaling semua objek jadi 0.6x (60% ukuran asli)
        scaleFactor = 0.6;
    } else {
        // Mode PC: Ukuran normal
        scaleFactor = 1.0;
    }
    
    // Update radius semua unit yang ada jika game sedang berjalan
    allUnits.forEach(u => {
        u.radius = baseUnitRadius * scaleFactor;
    });
}

// Jalankan saat pertama load dan saat layar di-resize
window.addEventListener('resize', adjustScaling);
adjustScaling(); // Initial call
// ========================================================
...
function update(time) { if (isPaused) return; const dt = time - lastTime; lastTime = time; ctx.clearRect(0, 0, canvas.width, canvas.height); let shakeX = (Math.random() - 0.5) * screenShake; let shakeY = (Math.random() - 0.5) * screenShake; ctx.save(); ctx.translate(shakeX, shakeY); const gs = allUnits.find(u => u.name === "Gojo" && u.isSkillActive); ctx.fillStyle = gs ? '#000000' : '#1e272e'; ctx.fillRect(arenaLeft, arenaTop, arenaRight - arenaLeft, arenaBottom - arenaTop); projectiles = projectiles.filter(p => !p.isDead); projectiles.forEach(p => { p.update(); p.draw(ctx); allUnits.forEach(u => { if (u.playerIdx !== p.ownerIdx && !u.isDead) { const d = Math.sqrt((u.x-p.x)**2+(u.y-p.y)**2); if (d < u.radius+p.radius) { u.applyDamage(p.dmg, 'shrine'); p.isDead=true; } } }); }); for (let i = 0; i < allUnits.length; i++) { for (let j = i + 1; j < allUnits.length; j++) { allUnits[i].checkCollision(allUnits[j]); } allUnits[i].update(dt); allUnits[i].draw(ctx); } ctx.restore(); if (gameStarted) updateUI(); if (gameStarted) { const p1_units = allUnits.filter(u => u.playerIdx === 0 && !u.isClone && !u.isDead); const p2_units = allUnits.filter(u => u.playerIdx === 1 && !u.isClone && !u.isDead); if (p1_units.length === 0 || p2_units.length === 0) { overlay.style.opacity = "1"; overlay.style.pointerEvents = "all"; let winnerName = ""; let winnerColor = "#fff"; if (p1_units.length > 0) { winnerName = p1_units[0].name.toUpperCase(); winnerColor = charColors[p1_units[0].name]; } else if (p2_units.length > 0) { winnerName = p2_units[0].name.toUpperCase(); winnerColor = charColors[p2_units[0].name]; } if (winnerName) { overlayMsg.innerHTML = `<span style="font-size: 48px; color: ${winnerColor}; text-shadow: 0 0 20px ${winnerColor}; font-weight: bold;">${winnerName} WIN</span>`; } else { overlayMsg.innerHTML = `<span style="font-size: 48px; color: #fff; font-weight: bold;">DRAW</span>`; } gameStarted = false; pauseBtn.style.display = "none"; } } animationId = requestAnimationFrame(update); }
