...
/**
 * FIXED AUDIO LOGIC v1.7.1
 * Mencegah browser "muting" karena overloading Audio Nodes
 */
function playSFX(audio, boost = 1) {
    if (!gameStarted || isPaused) return;
    
    // Gunakan clone sederhana tanpa AudioContext jika tidak butuh boost berlebih
    // Ini jauh lebih stabil untuk suara yang spamming (seperti domain/shinra tensei)
    const soundClone = audio.cloneNode();
    soundClone.volume = Math.min(1, audio.volume * boost);
    
    // Mainkan dan langsung buang dari memory setelah selesai
...
class Unit {
    ...
    update(deltaTime) {
        ...
        if (!this.isDead && !this.isClone) {
            ...
            if (this.name === "Pain") { 
                ...
                const r = this.isSkillActive ? 450 : 90; const p = this.isSkillActive ? 12.0 : (this.isPainPushing ? 12.0 : 4.0); 
                const interval = this.isSkillActive ? 600 : 400; 
                allUnits.forEach(u => { if (u.playerIdx !== this.playerIdx && !u.isDead) { const dx = this.x - u.x, dy = this.y - u.y, dist = Math.sqrt(dx*dx + dy*dy); if (dist < r + u.radius) { if (this.isSkillActive || this.isPainPushing) { u.x -= (dx/dist) * p; u.y -= (dy/dist) * p; } else { if (dist > this.radius) { u.x += (dx/dist) * p; u.y += (dy/dist) * p; } } if (this.gravityDmgTimer >= interval) u.applyDamage(2, 'gravity'); } } }); 
    ...
    draw(ctx) {
        ...
        if (this.isSkillActive && this.name === "Sukuna") { 
            ...
            const dashLen = 20 * scaleFactor; const gapLen = 15 * scaleFactor; ctx.setLineDash([dashLen, gapLen]);
            ctx.lineDashOffset = -this.shrineRotationOffset * (dashLen + gapLen); ctx.stroke(); ctx.restore();
